...
async function broadcast(rawTx) {
  ...
  /**
   * TAAL ARC rejects unauthenticated requests with 401, so including it
   * unconditionally guarantees a failed round trip and puts a misleading
   * "Authentication error" line in every broadcast failure. Only try it when a
