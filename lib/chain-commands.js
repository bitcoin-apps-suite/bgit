...
const CHAIN_COMMANDS = new Set(['publish', 'reconstruct', 'claim', 'verify', 'chain']);
...
async function routeChainCommand(command, argv) {
  ...
  switch (command) {
    ...
    case 'verify': return require('./verify-command').verifyCommand(withSourceDefaults(parseFlags(argv)));
