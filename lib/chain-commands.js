...
function parseFlags(argv) {
  const out = { bridges: [], _unknown: [] };
  const BOOLS = new Set(['broadcast', 'continue', 'confirm', 'quiet', 'yes', 'json', 'keep-bundle']);

  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (!arg.startsWith('--')) { out._unknown.push(arg); continue; }
    const name = arg.slice(2);
    const camel = name.replace(/-([a-z])/g, (_, c) => c.toUpperCase());

    if (BOOLS.has(name)) { out[camel] = true; continue; }
...
async function publishCommand(argv) {
  ...
  const outDir = flags.localOut || path.join(tempDir || process.cwd(), 'bgit-publish');
  ...
  try {
    ...
    if (!flags.yes) {
      const ok = await confirmPrompt(
        chalk.bold(`Broadcast ${dry.plan.txs ? dry.plan.txs.length : dry.files.length} transactions and spend up to ${formatSats(totalSats)}? [y/N] `)
      );
      if (!ok) {
